...
class ManagerSpatialHost (spatialHosts : List[ActorRef]) extends Actor {
  ...
  override def receive: Receive = {
    case AddClient(id : String, client : ActorRef) =>  {
      ...
      val playerData = PlayerData(id, Vector(250+0*rand.nextInt(500),500+0*rand.nextInt(1000)) :: List.empty[Vector], 50, rand.nextDouble(), 10, 10, Array(rand.nextInt(255), rand.nextInt(255), rand.nextInt(255)), null)
