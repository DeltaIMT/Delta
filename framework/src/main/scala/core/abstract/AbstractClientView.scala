...
import core.{Host, HostObserver, HostPool, HyperHost}
...
class  AbstractClientView(hosts: HostPool[_ <: Host, _ <: HostObserver], client: ActorRef) extends Actor {
  ...
  def zonesToMessage(zones: List[Zone]): Unit = {
    ...
    var hostInsideZones = List[HyperHost[_ <: Host]]()
    ...
    hostInsideZones.foreach({ h => {
      h.call(inside => {
        val res = inside.elements.values.filter(isElementInsideZones).toList
