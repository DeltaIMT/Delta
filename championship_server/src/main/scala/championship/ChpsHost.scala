...
class ChpsHost(zone: SquareZone) extends Host(zone) {
  ...
  def updateTarget(target:Boat,hunterId : String): Unit ={
    if (elements.contains(hunterId)){
      val hunter = elements(hunterId).asInstanceOf[Boat]
      hunter.fusionTargetId = target.id
      hunter.fusionTargetPosition = Option(target.position)
    }
  ...
  def lostPair(id : String) = {
    elements.get(id) match {
      case Some(b)=> {
        val boat = elements(id).asInstanceOf[Boat]
        boat.fusionTargetId=""
        boat.fusionTargetPosition = None
      }
      case None =>
    }
  }
  ...
  def tick(): Unit = {
    ...
    cannonBalls.foreach(cannonball => {
      ...
      if (cannonball.step){
        ...
        elements += id -> new Splash(cannonball.x, cannonball.y, id,"water")
      }
      ...
      boats.foreach(b => {
        b.collisionCannonBall(cannonball) match {
          case Some(damageOnBoat) => {
            ...
            elements -= cannonball.id
            val id = Random.alphanumeric.take(10).mkString
            elements += id -> new Splash(cannonball.x, cannonball.y, id,"boat")
          }
          case None => {}
        }
      })
      ...
      allObstacles.foreach( obs => {
        ...
        if ( (Vec(obs.x,obs.y)-cannonball.position).length < obs.maxsize &&   obs.shape.collision2(cannonball.getShape).isDefined  ){
          val id = Random.alphanumeric.take(10).mkString
          elements += id -> new Splash(cannonball.x, cannonball.y, id,"obstacle")
          elements -= cannonball.id
        }
    ...
    pairs.foreach { case List(b1, b2) => {
      if (b1.fusionTargetId == b2.id && (Vec(b1.x, b1.y) - Vec(b2.x, b2.y)).length < 100) {
        elements -= b1.id
        elements -= b2.id
        ...
        elements += id -> newBoat
        HP.hostObserver.call(hostobs =>{
          hostobs.notPairedYet += id -> (totalClient + totalClient % 2)
          hostobs.boats += newBoat.id -> newBoat
        })
