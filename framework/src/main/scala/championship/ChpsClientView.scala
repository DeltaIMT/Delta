  ...
  override def fromListToClientMsg(list: List[Any]) = {
    ...
    val listString = list.map {
          case e: Boat => {
            ...
            s"""{"type":"boat","id":"${e.id}","targets":${if(e.clientId2position.contains(id)) targetsString else "[]"},"health":[${e.health._1},${e.health._2}],"x":"${e.x.toInt}","y":"${e.y.toInt}","size":"${e.size}","ux":"${e.u.x}","uy":"${e.u.y}"}"""
          }
          case e: Cannonball => {
          ...
          case e: Splash => {
            s"""{"type":"splash","id":"${e.id}","x":"${e.x.toInt}","y":"${e.y.toInt}"}"""
          }
    } ++ List(
